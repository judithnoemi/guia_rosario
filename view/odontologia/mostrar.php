...
<body>
			...
			<div class="content">
				<div class="page-inner">
					<div class="page-header">
						<h4 class="page-title">Odontologia</h4>
											...
											<table id="add-row" class="display table table-striped table-hover">
												<thead>
													<tr>
														...
														<th>Atencion</th>
														<th>Diagnostico</th>
														<th>Primera_Consulta</th>
														<th>Pieza_Dental</th>
														<th>Mujeres_Emberazada</th>
														<th>Mujeres_Post_Parto</th>
														<th>Medidas_Preventidas</th>
														<th>Restauraciones</th>
														<th>Endodoncia</th>
														<th>Periodoncia</th>
														<th>Cirujia Bucal_Menor</th>
														<th>Otras_Acciones</th>
														<th>Rayos X</th>
														<th>Referencias y Contrareferencias</th>
														<th>Tratamiento</th>
														<th>Estado</th>
												...
												<tbody>
													...
													try {
														$sql = 'SELECT *
																...
																ORDER BY odontologia.idodontologia';
														foreach ($db->query($sql) as $row) {
													?>
															<tr>
																<td><?php echo $row['idodontologia']; ?></td>
																<td><?php echo $row['nombrep']." ".$row['apellidop']; ?></td>	
																<td><?php echo $row['nomdoc']." ".$row['apedoc']; ?></td>
																<td><?php echo $row['nombrees']; ?></td>
																<td><?php echo $row['fechaod']; ?></td>
																<td><?php echo $row['registrodon']; ?></td>
																<td><?php echo $row['detalleodon']; ?></td>
																<td><?php echo $row['atencionod']; ?></td>
																<td><?php echo $row['diagnosticood']; ?></td>
																<td><?php echo $row['primeraconod']; ?></td>
																<td><?php echo $row['piezadenod']; ?></td>
																<td><?php echo $row['mujeresemod']; ?></td>
																<td><?php echo $row['mujerespostod']; ?></td>
																<td><?php echo $row['medidasprevenod']; ?></td>
																<td><?php echo $row['restauracionesod']; ?></td>
																<td><?php echo $row['endodonciaod']; ?></td>
																<td><?php echo $row['periodonciaod']; ?></td>
																<td><?php echo $row['cir_bucalmenorod']; ?></td>
																<td><?php echo $row['otrasaccod']; ?></td>
																<td><?php echo $row['rxod']; ?></td>
																<td><?php echo $row['refycontrarefod']; ?></td>
																<td><?php echo $row['tratamientood']; ?></td>
																<td><?php echo $row['estado']; ?></td>

																<td>
																	<?php if ($row['estado'] == 1) { ?>
																		<form method="get" action="javascript:activo('<?php echo $row['idodontologia']; ?>')">
																			<button type="submit" class="btn btn-success btn-xs">Activo</button>
																		</form>

																	<?php  } else { ?>
																		<form method="get" action="javascript:inactivo('<?php echo $row['idodontologia']; ?>')">
																...
																<td>
																	<div class="form-button-action">
																		<button href="#editRowModal=<?php echo $row['idodontologia']; ?>" class="btn btn-link btn-primary btn-lg" data-toggle="modal" title="" data-original-title="Edit Task" data-target="#editRowModal<?php echo $row['idodontologia']; ?>">
																			<i class="fa fa-edit"></i>
																		</button>

																		<button href="#deleteRowModal=<?php echo $row['idodontologia']; ?>" class="btn btn-link btn-danger btn-lg" data-toggle="modal" title="" data-original-title="Delete Task" data-target="#deleteRowModal<?php echo $row['idodontologia']; ?>">
	...
	<script>
		function activo(idodontologia) {
			var id = idodontologia;
			$.ajax({
				type: "GET",
				url: "../assets/ajax/editar_estado_activo_odontologia.php?id=" + id,
			}).done(function(data) {
				window.location.href = '../folder/odontologia.php';
			})
		}

		// Editar estado inactivo
		function inactivo(idodontologia) {
			var id = idodontologia;
			$.ajax({
				type: "GET",
				url: "../assets/ajax/editar_estado_inactivo_odontologia.php?id=" + id,
			}).done(function(data) {
				window.location.href = '../folder/odontologia.php';
			})
		}
	...
	if (isset($_POST["agregar"])) {
		...
		$sql = "SELECT * FROM odontologia where idodontologia='$idodontologia'";
	...
	$sql2="INSERT INTO odontologia (codpaci, coddocod, fechaod, registrodon, detalleodon, atencionod, diagnosticood, primeraconod, piezadenod, mujeresemod, mujerespostod, medidasprevenod, restauracionesod, endodonciaod, periodonciaod, cir_bucalmenorod, otrasaccod, rxod, refycontrarefod, tratamientood, estado) 
			VALUES ('$codpaci', '$coddocod', '$fechaod', '$registrodon', '$detalleodon', '$atencionod','$diagnosticood','$primeraconod', '$piezadenod', '$mujeresemod','$mujerespostod', '$medidasprevenod', '$restauracionesod', '$endodonciaod', '$periodonciaod', '$cir_bucalmenorod', '$otrasaccod', '$rxod', '$refycontrarefod', '$tratamientood', '1');";	 
				...
				?>
					...
					<script type="text/javascript">
						Swal.fire({
							...
							window.location = "../folder/odontologia.php";
						});
					</script>
